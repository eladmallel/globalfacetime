/**
 * @license  OpenTok JavaScript Library v2.0.13.2
 * http://www.tokbox.com/
 *
 * Copyright (c) 2011 TokBox, Inc.
 *
 * Date: September 10 11:34:52 2013
 */
...
OT.properties = {
  version: "v2.0.13.2",         // The current version (eg. v2.0.4) (This is replaced by gradle)
  build: "660473e",    // The current build hash (This is replaced by gradle)
...
// Mount underscore somewhere handy
OT._ = _.noConflict();
...
// Change our default template settings
OT._.templateSettings = {
    evaluate    : /<%([\s\S]+?)%>/g,
    interpolate : /<%=([\s\S]+?)%>/g,
    escape      : /<%-([\s\S]+?)%>/g
};
...
OT.$.deepClone = function(obj) {
    if (!OT._.isObject(obj)) return obj;
    var newObj = OT._.isArray(obj) ? [] : {};
    for (var key in obj) {
        if (!obj.hasOwnProperty(key)) continue;
        if (OT._.isObject(obj[key])) {
...
OT.$.JSONify = (window.JSON && window.JSON.stringify) ? window.JSON.stringify : function (object) {
    // shallow JSONify the style property
    var styleString = "{ ";
...
OT.properties = function(properties) {
    var props = OT._.clone(properties);
...
// Calls +fn+ asynchronously with the current execution. This is most
// commonly used to execute something straight after the current function.
//
...
OT.$.eventing = function(self) {
  ...
  // @return Undefined
  //
  function executeListeners(listeners, args, name, defaultAction) {
    if (!listeners || listeners.length === 0) return;

    var listenerAcks = listeners.length;

    OT._.forEach(listeners, function(listener, index) {
  ...
  var removeAllListenersNamed = function (eventName, context) {
    if (_events[eventName]) {
      if (context) {
        // We are removing by context, remove all listeners that match
        // that context.
        _events[eventName] = OT._.reject(_events[eventName], function(listener){
  ...
  var removeListeners = function (eventNames, handler, context) {
    ...
    eventNames.forEach(function(name) {
      if (_events[name]) {
        _events[name] = OT._.reject(_events[name], filterHandlerAndContext);
  ...
  self.dispatchEvent = function(event, defaultAction) {
    ...
    executeListeners(_events[event.type], [event], event.type, defaultAction);
  ...
  self.trigger = function(eventName) {
    ...
    executeListeners(_events[eventName], args, eventName);
  ...
  // OT._.bind which return a new function everytime they are called.
  //
  ...
  self.off = function(eventNames, handler_or_context, context) {
    if (typeof(eventNames) === "string") {
      if (handler_or_context && OT._.isFunction(handler_or_context)) {
  ...
  self.one = function(eventNames, handler, context) {
        ...
        _this = {
            ...
            load: function(configUrl) {
                ...
                setTimeout(function() {
                    ...
                    _script.onload = _script.onreadystatechange = OT._.bind(_onLoad, this);
...
OT.setLogLevel = function(level) {
    ...
    if (shouldLog(OT.DEBUG) && !_debugHeaderLogged) {
        ...
        OT.debug("Release notes: " + OT.properties.websiteURL  +"/opentok/libraries/client/js/release-notes.html");
        OT.debug("Known issues: " + OT.properties.websiteURL + "/opentok/libraries/client/js/release-notes.html#knownIssues");
...
OT.$.getXML = function(url, options) {
    ...
    var extendedHeaders = OT._.extend(options.headers || {}, {
            'Content-Type': 'application/xml'
        });

    OT.$.get(url, OT._.extend(options || {}, {
        success: onSuccess,
...
OT.$.postFormData = function(url, data, options) {
    ...
    OT.$.post(url, OT._.extend(options || {}, {
        data: formData
    }));
};
...
function fixAspectRatio(element, width, height) {
    ...
    if (actualRatio > validAspectRatio) {
        // Width is largest so we blow up the height so we don't have letterboxing
        var newHeight = (actualRatio / validAspectRatio) * 100;

        props.height = newHeight + '%';
        props.top = '-' + ((newHeight - 100) / 2) + '%';
    } else if (actualRatio < validAspectRatio) {
        // Height is largest, blow up the width
        var newWidth = (validAspectRatio / actualRatio) * 100;
...
OT.WidgetView = function(domId, properties) {
    ...
    dimensionsObserver = OT.$.observeStyleChanges(container, ['width', 'height'], function(changeSet) {
        ...
        fixAspectRatio(videoContainer, width, height);
    });
    ...
    videoObserver = OT.$.observeNodeOrChildNodeRemoval(container, function(removedNodes) {
        ...
        // not be the case.
        var videoRemoved = OT._.some(removedNodes, function(node) { return node === videoContainer || node.nodeName === 'VIDEO'; });
...
//
OT.$.getUserMedia = function(constraints, success, failure, accessDialogOpened, accessDialogClosed) {
    // All constraints are false, we don't allow this
    if (!constraints || OT._.all(constraints, function(constraint){ return constraint === false; })) {
        OT.error("Couldn't get UserMedia: All constraints were false");
        // Using a ugly dummy-code for now.
        failure({code: 'NO_CONSTRAINTS'});
        return;
    }
...
OT.VideoElement = function(options) {
    var _stream,
        ...
        _parentElement,
        _streamBound = false,
        _options = OT._.defaults(options || {}, {
    ...
    /// Private API
    var _onVideoError = OT._.bind(function(event) {
        ...
        _onStreamBound = OT._.bind(function() {
            _streamBound = true;
            this.trigger('streamBound', this);
        }, this),

        _onTimeout = OT._.bind(function(reason) {
            this.trigger('error', OT.ExceptionCodes.P2P_CONNECTION_FAILED, reason, this);
        }, this);
    ...
    this.bindToStream = function(webRtcStream) {
        ...
        bindStreamToVideoElement(_domElement, _stream, _onStreamBound, _onTimeout);
    ...
    this.setAudioVolume = function(value) {
        if (_domElement) OT.$.roundFloat(_domElement.volume = value / 100, 2);
    };
...
if (OT.$.canDefineProperty) {
    ...
    Object.defineProperty(OT.VideoElement.prototype,'imgData', {
        get: function() {
...
function bindStreamToVideoElement(videoElement, webRTCStream, onStreamBound, onTimeout) {
    ...
    if (navigator.mozGetUserMedia || (webRTCStream.getVideoTracks().length > 0 && webRTCStream.getVideoTracks()[0].enabled)) {
        ...
        var timeout = setTimeout(OT._.bind(function() {
            if (videoElement.currentTime === 0) {
                onTimeout("The video stream failed to connect. Please notify the site owner if this continues to happen.");
            } else {
    ...
    var endPoint = OT.properties.loggingURL + "/logging/ClientEvent",
        ...
        this.logError = function(code, type, message, details, options) {
            ...
            var errKey = [partnerId, type, code].join('_'),
                ...
                payload = this.escapePayload(OT._.extend(details || {}, {
                    message: payload,
            ...
            return this.logEvent(OT._.extend(options, {
                action: type + '.' + code,
        ...
        this.logEvent = function(options) {
            ...
            var data = OT._.extend({
                    "variation" : "",
                    'guid' : options.connectionId || '',
                    'widget_id' : "",
                    'session_id': '',
                    'connection_id': '',
                    'stream_id' : "",
...
OT.initPublisher = function(apiKey, replaceElementId, properties) {
    ...
    if (success) publisher.one('initSuccess', success);
    if (error) publisher.one('publishError', error);
...
OT.WebSocketMessage.signalMessage = function(sessionId, fromConnectionId, options) {
    ...
    this.onSuccess = message.success && OT._.isFunction(message.success) ? OT.$.createAsyncHandler(message.success) : null;
    this.onError = message.error && OT._.isFunction(message.error) ? OT.$.createAsyncHandler(message.error) : null;
    ...
    var isInvalidType = function(type) {
        ...
        validateTo = function() {
            ...
            else if ( OT._.isArray(message.to) ) {
    ...
    if (this.valid) {
        ...
        if (message.to && !OT._.isArray(message.to)) {
        ...
        var payload = {
            id: this.id,
            toAddresses: OT._.map(this.signal.to, function(thing) { return typeof(thing) === 'string' ? thing : thing.id; }),
...
OT.Messenger = function(messagingServer, messageWrangler) {
    ...
    var WEB_SOCKET_SERVER_PORT = 443;
    ...
    var logAnalyticsEvent = function(variation, payloadType, payload, options) {
        ...
        if (options) event = OT._.extend(options, event);
    ...
    this.connect = function (sessionId, sessionToken, options) {
        ...
        _webSocketUrl = "wss://" + messagingServer + ":" + WEB_SOCKET_SERVER_PORT + "/rumorwebsockets";
...
function _exceptionHandler(component, msg, errorCode, context) {
    var title = errorsCodesToTitle[errorCode],
        contextCopy = context ? OT._.clone(context) : {};
...
OT.DOMComponentCleanup = (function () {
    ...
    return {
        ...
        remove: function(component) {
            ...
            if ( (index = OT._.indexOf(_windows[win], component)) != -1) {
...
OT.PeerConnection = function(config) {
    ...
    // Private methods
    var delegateMessage = OT._.bind(function(type, messagePayload) {
        ...
        setupPeerConnection = OT._.bind(function() {
            if (!_peerConnection) {
                try {
                ...
                _peerConnection.onaddstream = OT._.bind(onRemoteStreamAdded, this);
                _peerConnection.onremovestream = OT._.bind(onRemoteStreamRemoved, this);

                if (_peerConnection.onsignalingstatechange !== undefined) {
                    _peerConnection.onsignalingstatechange = OT._.bind(routeStateChanged, this);
                } else if (_peerConnection.onstatechange !== undefined) {
                    _peerConnection.onstatechange = OT._.bind(routeStateChanged, this);
                }
            }
        ...
        generateErrorCallback = function(forMethod, message) {
            return OT._.bind(function(errorReason) {
        ...
        triggerError = OT._.bind(function(errorReason) {
    ...
    this.unregisterMessageDelegate = function(delegateFn) {
        OT._.without(_messageDelegates, delegateFn);
    };
...
OT.PublisherPeerConnection = function(remoteConnection, session, stream, webRTCStream) {
    ...
    var _onPeerClosed = function() {
        ...
        _relayMessageToPeer = OT._.bind(function(type, payload) {
    ...
    _peerConnection.on({
        close: OT._.bind(_onPeerClosed, this),
        error: OT._.bind(_onPeerError, this)
    });
...
OT.SubscriberPeerConnection = function(remoteConnection, session, stream, properties) {
    ...
    _peerConnection.on({
        close: OT._.bind(_onPeerClosed, this),
        streamAdded: OT._.bind(_onRemoteStreamAdded, this),
        streamRemoved: OT._.bind(_onRemoteStreamRemoved, this),
        error: OT._.bind(_onPeerError, this)
    });
...
OT.Chrome.NamePanel = function(options) {
    ...
    this.__defineSetter__("name", OT._.bind(function(name) {
...
OT.Chrome.MuteButton = function(options) {
    ...
    var attachEvents = function(elem) {
            _onClickCb = OT._.bind(onClick, this);
    ...
    OT.Chrome.Behaviour.Widget(this, {
        ...
        onCreate: OT._.bind(attachEvents, this),
        onDestroy: OT._.bind(detachEvents, this)
    });
};
...
OT.Chrome.SettingsPanel = function(options) {
    ...
    OT.Chrome.Behaviour.Widget(this, {
        ...
        htmlContent: 'Settings',
        htmlAttributes: {className: 'OT_settings-panel'},
        onCreate: OT._.bind(renderDialog, this),
        onShow: OT._.bind(onShow, this)
    });
};
        ...
        isValidStyle = function(key, value) {
            return key === 'backgroundImageURI' ||
                    (   _validStyleValues.hasOwnProperty(key) &&
                        OT._.include(_validStyleValues[key], value) );
        },
...
OT.generateSimpleStateMachine = function(initialState, states, transitions) {
  var validStates = states.slice(),
      validTransitions = OT._.clone(transitions);
...
var validStates = [ 'NotPublishing', 'GetUserMedia', 'BindingMedia', 'MediaBound', 'PublishingToSession', 'Publishing', 'Failed' ],
  ...
  validTransitions = {
     ...
     Publishing: ['MediaBound', 'NotPublishing', 'Failed'],
     Failed: []
   },
...
OT.Publisher = function() {
    ...
    var logAnalyticsEvent = function(action, variation, payloadType, payload) {
        ...
        onStreamAvailable = function(webOTStream) {
            ...
            _targetElement.on({
                    streamBound: OT._.bind(onLoaded, this),
                    timeout: OT._.bind(onLoadFailure, this),
                    error: OT._.bind(onVideoError, this)
                })
        ...
        onStreamAvailableError = function(error) {
            var reason,
                ...
                handleJsException = function(errorMessage, self) {
                    OT.error('OT.Publisher.onStreamAvailableError ' + errorMessage);

                    logAnalyticsEvent('publish', 'Failure', 'reason', "Publisher failed to access camera/mic: " + errorMessage);

                    OT.handleJsException("Publisher failed to access camera/mic: " + errorMessage, 2000, {
                        session: _session,
                        target: self
                    });
                };
            ...
            case 1:
                ...
                reason = "Publisher Access Denied: Permission Denied (" + error.code + ')';
                logAnalyticsEvent('publish', 'Failure', 'reason', reason);
                ...
                this.trigger('publishError', reason);
                this.dispatchEvent(event, defaultAction);

                break;

            case 'NO_CONSTRAINTS':
                reason = 'No Media constaints were provided';
                this.trigger('publishError', reason);

                if (_container) _container.destroy();
                handleJsException(reason, this);
                break;

            default:
                reason = 'Unknown error while getting user media';
                this.trigger('publishError', reason);
        ...
        createPeerConnectionForRemote = function(remoteConnection) {
            ...
            if (!peerConnection) {
                ...
                peerConnection.on({
                    ...
                    disconnected: OT._.bind(onPeerDisconnected, this),
                    error: OT._.bind(onPeerConnectionFailure, this)
                });
            }
    ...
    this.publish = function(domId, properties) {
        ...
        _publishProperties = OT._.defaults(properties || {}, {
            publishAudio: true,
            publishVideo: true,
            mirror: true
        });
        ...
        OT.onLoad(function() {
            ...
            if (isValidResolution(_publishProperties.encodedWidth, _publishProperties.encodedHeight)) {
                constraints.video = { mandatory: {
                    maxWidth: _publishProperties.encodedWidth,
                    minWidth: _publishProperties.encodedWidth,
                    maxHeight: _publishProperties.encodedHeight,
                    minHeight: _publishProperties.encodedHeight
                } };
            }
            else {
                delete _publishProperties.encodedWidth;
                delete _publishProperties.encodedHeight;
            }
            OT.$.getUserMedia(
                constraints,
                OT._.bind(onStreamAvailable, this),
                OT._.bind(onStreamAvailableError, this),
                OT._.bind(onAccessDialogOpened, this),
                OT._.bind(onAccessDialogClosed, this)
            );
        }, this);
  ...
  */
    ...
    */
    ...
    this._ = {
        publishToSession: function(session) {
            ...
            var createStream = function() {
                ...
                var width = _publishProperties.encodedWidth || 640;
                var height = _publishProperties.encodedHeight || 480;

                _state.set('PublishingToSession');

                session.sendMessage(
                    OT.WebSocketMessage.createStream(this.guid, _publishProperties && _publishProperties.name ? _publishProperties.name : "", OT.VideoOrientation.ROTATED_NORMAL, width, height, _publishProperties.publishAudio, _publishProperties.publishVideo, this.session.sessionInfo.p2pEnabled)
                );
            };

            if (_loaded) createStream.call(this);
            else this.on("initSuccess", OT._.bind(createStream, this));
    ...
    this.on('styleValueChanged', OT._.bind(updateChromeForStyleChange, this));
    _state = new OT.PublishingState(stateChangeFailed);

    this.__defineGetter__("id", function() { return _domId; });
    this.__defineGetter__("guid", function() { return _guid; });

    this.__defineGetter__("stream", function() { return _stream; });
    this.__defineSetter__("stream", function(stream) { _stream = stream; });
    ...
    this.__defineGetter__("targetElement", function() { return _targetElement.domElement; });
    this.__defineGetter__("domId", function() { return _domId; });

    this.__defineGetter__("session", function() { return _session; });
    this.__defineSetter__("session", function(session) { _session = session; });

    this.__defineGetter__("isWebRTC", function() { return true; });
...
OT.Subscriber = function(replaceElementId, options) {
    var _widgetId = uuid(),
        ...
        _qosInterval,
        _properties = OT._.clone(options),
        _analytics = new OT.Analytics(),
        _audioVolume = 50,
        _gettingStats = 0,
        _state;
    ...
    var logAnalyticsEvent = function(action, variation, payloadType, payload) {
        ...
        onRemoteStreamAdded = function(webOTStream) {
            ...
            streamElement.on({
                    streamBound: OT._.bind(onLoaded, this),
                    timeout: OT._.bind(onPeerConnectionFailure, this),
                    error: OT._.bind(onPeerConnectionFailure, this)
                });
    ...
    this.subscribe = function(stream) {
        ...
        if (_stream.connection.id !== _session.connection.id) {
            ...
            _peerConnection.on({
                disconnected: OT._.bind(onDisconnected, this),
                error: OT._.bind(onPeerConnectionFailure, this),
                remoteStreamAdded: OT._.bind(onRemoteStreamAdded, this),
                remoteStreamRemoved: OT._.bind(onRemoteStreamRemoved, this)
            });
        }
        else {
            // Subscribe to yourself edge-case
            onRemoteStreamAdded.call(this, _session.getPublisherForStream(_stream)._.webRtcStream, "merged");
        }
    ...
    */
    ...
    this.__defineGetter__("id", function() { return _domId; });
    this.__defineGetter__("stream", function() { return _stream; });
    this.__defineGetter__("targetElement", function() { return _streamContainer ? _streamContainer.domElement : null; });
    this.__defineGetter__("subscribing", function() { return _state.subscribing; });
...
(function(window) {
    ...
    OT.Capabilities = function(permissions) {
        this.publish = OT._.contains(permissions, "publish") ? 1 : 0;
        this.subscribe = OT._.contains(permissions, "subscribe") ? 1 : 0;
        this.forceUnpublish = OT._.contains(permissions, "forceunpublish") ? 1 : 0;
        this.forceDisconnect = OT._.contains(permissions, "forcedisconnect") ? 1 : 0;
...
OT.Session = function(sessionId) {
  ...
  function updateStreamProperty(streamId, key, newValue) {
    ...
    packet.streams = OT._.filter(packet.streams, function(stream){
  ...
  sessionConnectFailed = function(reason, code) {
    ...
    disconnectComponents.call(this);
      ...
      if (!event.isDefaultPrevented()) destroyComponents.call(this);
        ...
        this.dispatchEvent(event, OT._.bind(defaultAction, this));
    },
  ...
  streamCreatedHandler = function(packet) {
    // Reject any streams that are against connections that we don't recognise,
    // or ones that the user shouldn't be notified about.
    packet.streams = OT._.filter(packet.streams, validConnectionOnStream);
...
//          return;
  ...
  cleanupPublisherOnDestroy = function(publisher) {
    _publisherCleanupEvents[publisher.guid] = OT._.bind(function() {
    ...
    _messenger = new OT.Messenger(
      ...
      _wrangler
        ...
        SessionConnected: _bind(sessionConnectedHandler),
        SessionConnectFailed: _bind(sessionConnectionFailed),
        ConnectionClosed: _bind(sessionDisconnectedHandler),
        ConnectionCreated: _bind(connectionCreatedHandler),
        ConnectionDestroyed: _bind(connectionDestroyedHandler),
        StreamRegistered: _bind(streamRegisteredHandler),
        StreamCreated: _bind(streamCreatedHandler),
        StreamModified: _bind(streamPropertyModifiedHandler),
        StreamDestroyed: _bind(streamDestroyedHandler),
        JSEPMessageReceived: _bind(jsepMessageHandler),
        SignalReceived: _bind(signalHandler),
        SubscriberVideoDisabled: _bind(subscriberVideoDisabledHandler),
        exception: _bind(exceptionHandler)
    });
 ...
 *
  ...
  */
  this.connect = function(apiKey, token, properties) {
    ...
    if (properties && properties.success) this.one(OT.Event.names.SESSION_CONNECTED, properties.success);
    if (properties && properties.error) this.one('sessionConnectFailed', properties.error);
  ...
  */
  this.publish = function(publisher, properties) {
    ...
    if (!publisher || typeof(publisher)==='string'){
     ...
     publisher = OT.initPublisher(this.apiKey, publisher, properties);
      ...
      if (properties && properties.error) {
        ...
        publisher.one('publishError', properties.error);
      }
    }
    else {
    ...
    if (success) publisher.one('publishSuccess', success);
  ...
  */
  this.subscribe = function(stream, replaceElementId, properties) {
    ...
    var subscriber = new OT.Subscriber(replaceElementId, OT._.extend(properties || {}, {
        session: this
    }));

    if (properties && properties.success) subscriber.one('subscribeSuccess', properties.success);
    if (properties && properties.error) subscriber.one('subscribeError', properties.error);
  ...
  * @memberOf Session
  */
  this.forceDisconnect = function(connectionOrConnectionId) {
